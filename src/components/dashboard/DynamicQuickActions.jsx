...
const ROLE_CONFIGS = {
  admin: {
    ...
    actions: [
      ...
      { label: 'Matières & Cours', icon: FileText, href: '/matieres-cours', color: 'bg-green-50 text-green-600 border-green-100' },
      { label: 'Emploi du Temps', icon: Clock, href: '/emploi-du-temps', color: 'bg-purple-50 text-purple-600 border-purple-100' },
...
function QuickActionsGrid({ config, onAction }) {
  return (
...
function AdminQuickActions() {
  ...
  return (
    <>
      <QuickActionsGrid
        config={ROLE_CONFIGS.admin}
        onAction={(actionKey) => {
          if (actionKey === 'createUser') createModal.open();
        }}
      />
...
function ProfesseurQuickActions() {
  return <QuickActionsGrid config={ROLE_CONFIGS.professeur} />;
}

export default function DynamicQuickActions({ role = 'admin' }) {
  if (role === 'professeur') {
    return <ProfesseurQuickActions />;
  }
